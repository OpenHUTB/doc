...
2021 年，基于驾驶员认知行为克隆（克隆DCB）的生态交通和数字交通项目为自动驾驶项目提供资金。在这个项目中，我们深入研究了端到端的模式，将人类驾驶行为作为训练依据。

该项目的一个重要方面是采集人类驾驶数据，实现对实验场景的最大管控。项目依托 CARLA 模拟器完成数据采集与实验场景管控。

Carla 允许定义轨道交通（交通、天气、气象等）的各种情况，并获得驾驶员、车辆速度等仿真场景数据。与驾驶员相关的物理传感数据，与仿真数据时序同步采集。

事实上，这些行为在虚拟预测和控制变量的使用场景中起着重要作用。驾驶员可在驾驶舱获得感官沉浸体验，设备可模拟车辆运动（如加速、制动等）。
...
* 提供数据收集所需的基础设施和人员。
* 负责遵守 LOPD 关于实验驾驶员数据的规定，及时通知他们并征得他们的同意。
...
* 模拟器还必须显示两个相当于外部后视镜的视图（为本项目强制配置要求）。
...
只有符合上述要求的实体才能申请本次招标。如果有多个候选人，将评估以下几点以选出获胜者：
...
$$ P5 = \frac{maximo(1, PFmax-OF)}{maximo (1, OFmax-OFmin)} \times 5 $$
...
实体必须获得最低分数 70 分才有资格获得合同，即：P1 + P2 + P3 + P4 + P5 + P6 ≥ 70。在此过程中，每个候选实体有责任完整、简洁、可核验地提交，CVC计算部分分数所需的信息。未提交的信息将不予考虑。

## 合同预算

合同预算为 45,000 欧元（不含增值税），允许最高上浮 10% 的金额调整。合同必须在授予后最多 5 个月内执行。

## 报名截止日期

自要约在 CVC 网站上发布之日起，该要约将持续 10 个日历日。投标方需标注编号 SERV20230016 并提交投标文件至 [链接](https://www.cvc.uab.es/send-us-an-offer/) 。
